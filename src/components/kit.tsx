...
export function StepperHeader({ steps, current }: { steps: readonly string[]; current: number }) {
  return (
    <div className="mb-5">
      ...
      <div className="mt-2 flex justify-between">
        {steps.map((step, i) => (
          <span
            key={step}
            className={cn(
              "flex-1 text-center text-[10px] font-medium first:text-left last:flex-none last:text-right",
...
export function LoadingOverlay({ label = "Please wait…" }: { label?: string }) {
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center bg-background/70 backdrop-blur-[2px]">
      <div className="rounded-xl border border-border bg-card px-4 py-3 text-sm font-medium text-foreground shadow-sm">
        {label}
      </div>
    </div>
  );
...
export function ConfirmDialog({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/50 px-6" onClick={onClose}>
      <div
        role="dialog"
        aria-modal
        aria-labelledby="confirm-title"
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-[360px] rounded-2xl border border-border bg-card p-5 shadow-lg"
      >
        <h3 id="confirm-title" className="font-display text-lg font-semibold text-foreground">
          {title}
        </h3>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{body}</p>
        <div className="mt-5 flex gap-2">
          <Button type="button" variant="outline" full onClick={onClose} disabled={busy}>
            Keep
          </Button>
          <Button type="button" variant={danger ? "danger" : "primary"} full onClick={onConfirm} disabled={busy}>
...
export function Banner({
  ...
  return <div className={cn("rounded-xl border px-3 py-2.5 text-sm font-medium", styles)}>{children}</div>;
}
      ...
      <div className="flex w-full max-w-[448px] flex-col gap-2">
        {toasts.map((t) => (
          <button
            ...
            className="pointer-events-auto rounded-xl border border-border bg-card px-4 py-3 text-left shadow-lg"
          >
